...
#create mtd table data to hold coupons
try:
    df_coup=pd.read_sql("""select coup_id
    from sales_fact_dtl d
where coup_id not in(select * from t_pos_coup_tndr)
and BUS_DATE between date_trunc('month', add_months(current_date,-1)) and last_day(add_months(current_date,-1))
...
try:
    corp2.execute("delete from crm@dw_prd:uac_coup_lm")
    logger.info('removed data from mtd table.')
except:
...
and d.IS_GOSHP_ID=0
and d.STORE_ID<>9000
and d.BUS_DATE between date_trunc('month', add_months(current_date,-1)) and last_day(add_months(current_date,-1))
...
for y in x[0:]:
    ...
    query="""insert into 
    crm@dw_prd:uac_coup_lm(item_id, coupon_description, revenue, item_cost, margin_dlr, margin_pct, total_transactions, 
