...
and BUS_DATE between (current_date - extract(dow from current_date)+1)-7 and (current_date - extract(dow from current_date)+1)-1
group by 1"""
...
try:
    corp2.execute("delete from crm@dw_prd:uac_coup_lw")
    logger.info('removed data from lw table.')
except:
    logger.info('unable to remove data from lw table.')
...
for y in x[1:]:
    ...
    query="""insert into 
    crm@dw_prd:uac_coup_lw(item_id, coupon_description, revenue, item_cost, margin_dlr, margin_pct, total_transactions, 
    qty_redeemed, coupon_cost, start_date, end_date) values({y1},'{y2}',{y3},{y4},{y5},{y6},{y7},{y8},{y9},{y10},{y11})"""
    query=query.format(**locals())
    try:
        corp2.execute(query)
    except:
        print(y1+ ' and '+ y2 + ' did not work')
